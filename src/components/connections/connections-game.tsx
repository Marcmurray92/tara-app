...
import type { ConnectionsGameData, ConnectionsProgress, ConnectionsSubmitFeedback } from "@/features/connections/game/connections-game.types";
import {
      ...
      <div className="rounded-[1.5rem] border border-white/10 bg-surface/90 p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            <Badge className="w-fit">Movie mode</Badge>
            <div>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm text-muted">
              Solved {progress.solvedGroupIds.length}/4
            </div>
            <div className="rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm text-muted">
              Mistakes left {Math.max(0, 4 - progress.mistakes)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {unsolvedTiles.map((tile) => (
              <button
                ...
                className={cn(
                  "min-h-24 rounded-[1.25rem] border px-4 py-5 text-center text-sm font-medium leading-6 transition",
                  selectedIds.has(tile.id)
                    ? "border-accent bg-accent text-background shadow-glow"
                    : "border-white/10 bg-surface/90 text-text hover:border-accent/45 hover:bg-surface-strong"
                )}
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button onClick={handleSubmit} disabled={progress.selectedItemIds.length !== 4 || progress.status !== "playing"}>
          ...
          <div className="rounded-[1.25rem] border border-white/10 bg-surface/90 p-4 text-sm leading-7 text-muted">{message}</div>
        </div>
              ...
              <CardContent className="space-y-3 text-sm text-muted">
                {remainingGroups.map((group) => (
                  <div key={group.id} className={cn("rounded-xl border p-3", difficultyTone(group.difficulty))}>
                    ...
                    <p className="mt-2 leading-6">{group.items.join(" • ")}</p>
                  </div>
                ))}
              </CardContent>
            </Card>
          ...
          {progress.status === "won" ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>You found every set on this device.</CardDescription>
              </CardHeader>
            </Card>
          ) : null}
        </aside>
